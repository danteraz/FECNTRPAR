...
export default function ParticipanteQRCode() {
  ...
  const fetchParticipantes = async () => {
    console.log("Passando FONE LIMPO",fone)
    ...
    try {
      ...
      console.log("Verificando retorno do FONE: ",participante);
      ...
      if (response.status === 400) {
        ...
        console.log("Enviando Participante para Verificar: ",participante.idParticipante);
  ...
  const verificarInscricao = async (idParticipante) => {
    try {
      ...
      console.log("RETORNO DA VERIFICAÇÂO DE PRESENÇA ",response);

      if (response.ok) {
        const data = await response.json(); // Obtenha os dados da resposta
        console.log("RETORNO DA VERIFICAÇÃO DE PRESENÇA", data); // Exibe o conteúdo da resposta
  
        if (data.length > 0) {
          setStatusInscricao('JÁ inscrito');
          setInputDisabled(true); // Desabilita a navegação para manutenção dos participantes
          setMostraBotao("JÁ")
          console.log("Você JÁ Está Inscrito nesta Palestra!");
        } else {
          setStatusInscricao("NÃO inscrito");
          setInputDisabled(false); // Desabilita a navegação para manutenção dos participantes
          setMostraBotao("NÃO")
          console.log("Você NÃO Está Inscrito nesta Palestra!");
        }
      } else {
  ...
  const confirmarOuRemoverInscricao = async () => {
    if (statusInscricao.includes("NÃO")) {
      ...
      try {
        if (idParticipante) {
          console.log("ATUALIZA PARTICIPANTE = ",idParticipante)
          ...
          await fetch(`/api/participantes/${idParticipante}`, {
            ...
            body: JSON.stringify({ nome, foneLimpo, email, mensagem }),
          });
        } else {
          console.log("INCLUI PARTICIPANTE = ",idParticipante)
        ...
        console.log("INSCRIÇÃO PARTICIPANTE/PALESTRA = ",idParticipante,idPalestra)
      ...
      // Remover Inscrição
      console.log("REMOVER PARTICIPANTE/PALESTRA = ",idParticipante,idPalestra)
      try {
